...
const PRELOADS = [
    {name: 'diet', tokenURI: 'https://picsum.photos/700'},
];
...
describe("TLDToken", () => {
    ...
    describe("constructor", () => {
        it("should premint tokens passed to the constructor", async () => {
            for(let preload of PRELOADS) {
                const label = getTokenId(preload.name);
                expect(await token.ownerOf(label)).to.equal(signers[1].address);
                expect(await token.tokenURI(label)).to.equal(preload.tokenURI);
            }
        });
    ...
    describe("tokenURI", () => {
        it("should return a token URI for existing tokens", async () => {
            for(const preload of PRELOADS) {
                expect(await token.tokenURI(getTokenId(preload.name))).to.equal(preload.tokenURI);
            }
        });
    ...
    describe("burn", () => {
        ...
        it("should allow minting a token that has been burned", async () => {
            const id = getTokenId(PRELOADS[0].name);
            await token.burn(id);
            await expect(token.mintTLD(PRELOADS[0].name, signers[1].address, PRELOADS[0].tokenURI))
                .to.emit(token, 'Transfer');
        });
    });
});
